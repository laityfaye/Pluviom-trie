...
services:
  ...
  timescaledb:
    ...
    environment:
      - POSTGRES_DB=${POSTGRES_DB:-climatsn_db}
      - POSTGRES_USER=${POSTGRES_USER:-postgres}
      - POSTGRES_PASSWORD=${POSTGRES_PASSWORD:-rout}
    ...
    ports:
      - "5432:5432"
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U $${POSTGRES_USER:-postgres} -d $${POSTGRES_DB:-climatsn_db}"]
      interval: 30s
      timeout: 10s
      retries: 3
  ...
  ml-pipeline:
    ...
    environment:
      - DATABASE_URL=postgresql://$${POSTGRES_USER:-postgres}:$${POSTGRES_PASSWORD:-rout}@timescaledb:5432/$${POSTGRES_DB:-climatsn_db}
